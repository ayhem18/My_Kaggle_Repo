...
from building_blocks import LinearNet


class DistanceMatrixGenerator(nn.Module):
    """This class is responsible for generating the distance matrix given a set of features / embeddings / samples.
    The class passes the visual embeddings generated by the Resnet architecture (consider the 'Technical Details' section in the paper) 
    through a FeedForward Neural Network and computes cosine difference between the resulting embeddings.
    """
    def __init__(self, 
                 hidden_units: List[int], 
                 batch_normalization: Union[List[bool], bool], 
                 dropout_rate: Union[List[float], float], 
                 ) -> None:
        super().__init__()
        # first initialize the inner linear network
        self.linear_network = LinearNet(hidden_units=hidden_units, 
                                        batch_normalization=batch_normalization, 
                                        dropout_rate=dropout_rate, 
                                        last_layer_is_final=True)
        

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """This function generates the difference matrix between the embeddings in the given batch 

        Args:
            x (torch.Tensor): a batch of embeddings

        Returns:
            torch.Tensor: Cosine distance matrix between the different embeddings 
        """
        # let's first squeeze the batch
        x = torch.squeeze(x, dim=0)
        # make sure the data is still 2 dimensional
        if x.dim() != 2:
            raise ValueError(f"The data is expected to be 2 dimensionsal.\nFound: {x.dim()} dimensions")

        # pass the data through the classifier
        x = self.linear_network(x)

        # normalize each vector
        x_norm = x / torch.norm(x, dim=0)
        # make sure the normalization was carried out correctly
        assert torch.isclose(np.ones(shape=(len(x_norm), 1)), torch.sum(x_norm, dim=0)), "There might be an issue with normalization"

        # The i-th row of the matrix should represent the difference between the i-th sample and all other samples.
        distance_matrix = x_norm @ x_norm.T
